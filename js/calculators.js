(() => {
  ...
  const calc = {
    ...
    car: () => loan("price", "down", "rate", "months", false),
    mortgage: () => loan("price", "down", "rate", "years", true),
    time: () => {
  ...
  if (calc[key]) {
    ...
    if (key === "mattress") $("size")?.addEventListener("change", calc[key]);
  }
})();
