(() => {
  ...
  const additionalMedicareThreshold2026 = {
    single: 200000,
    married: 250000,
    hoh: 200000,
  };
  ...
  if (calc[key]) {
    ...
    if (key === "zip")
      $("zip")?.addEventListener("keydown", (e) => {
        if (e.key === "Enter") calc.zip();
      });
    if (key === "area")
      $("areaCode")?.addEventListener("keydown", (e) => {
        if (e.key === "Enter") calc.area();
      });
  }
})();
