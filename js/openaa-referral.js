(() => {
  ...
  const groups = [
    {
      ...
      description:
        "OpenAA 汇集美国华人招聘与求职信息，方便继续比较职位和收入。",
      label: "查看附近招聘",
    },
    {
      match: [
  ...
  function buildHomepageEntry() {
    ...
    section.innerHTML =
      '<div><div class="cta-eyebrow">OpenAA · 美国华人生活</div><h2>工具算完了，生活信息接着找</h2><p>按你现在要办的事情直接进入，不用重新搜索。</p></div><div class="openaa-entry-links"><a data-destination="/jobs">找工作</a><a data-destination="/housing">找房屋</a><a data-destination="/marketplace">二手市场</a><a data-destination="https://dmv.openaa.com">DMV题库</a></div>';
  ...
  function buildContextualEntry() {
    ...
    if (toolPage && toolPage.contains(section)) {
      ...
      const note = result?.nextElementSibling?.classList.contains("note")
  ...
  function updateOtherLinks() {
    document
      .querySelectorAll(
        'a[href*="openaa.com"]:not(.openaa-nav-link):not(.openaa-next-step a):not(.openaa-home-entry a)',
      )
      .forEach((link) => {
        ...
        try {
          ...
          destination =
            url.hostname === "openaa.com" ? url.pathname || "/" : url.href;
        } catch (_) {}
  ...
  function init() {
    ...
    updateHeader();
    if (path === "/" || path === "/tools/" || path === "/tools/index.html")
      buildHomepageEntry();
