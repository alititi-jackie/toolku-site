...
function run(code, tool, values, advanced = false) {
  ...
  const context = {
    document,
    window: { location: { href: "https://toolku.com/" } },
    console,
    Intl,
    Date,
    Math,
  ...
  vm.runInNewContext(code, context, {
    filename: advanced ? "new-calculators.js" : "calculators.js",
  });
  assert.equal(
    typeof elements.calculate.onclick,
    "function",
    tool + " did not register click handler",
  );
...
for (const [tool, input, expected] of standard)
  includes(run(standardCode, tool, input).html, expected, tool);
...
for (const [tool, input, expected] of advanced)
  assert.equal(run(advancedCode, tool, input, true).value, expected, tool);
...
assert.equal(
  new Set(catalog.map((x) => x[0])).size,
  catalog.length,
...
for (const item of catalog) {
  ...
  assert.ok(
    fs.existsSync("tools/" + item[0] + "/index.html"),
    "missing page for " + item[0],
  );
}
...
for (const app of ["expense-record", "usd-rmb"]) {
  ...
  assert.match(
    html,
    /onclick="shareOpenAA\(\)"/,
    app + " share button must call shareOpenAA",
  );
  assert.match(
    js,
    /async function shareOpenAA\(\)/,
    app + " must define shareOpenAA",
  );
}
